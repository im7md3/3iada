...
trait JodaResource
{
    ...
    public function index()
    {
        ...
        $externalData = $this->external('index');
        $index = ${$this->pluralCamelName};
        $route = $this->route;
        $title = trans(ucfirst($this->pluralCamelName));
        return view("{$this->view}.index", array_merge(compact($this->pluralCamelName, 'index', 'route', 'title'), $externalData));
    }
    ...
    {
        $route = $this->route;
        $externalData = $this->external('create');
        $title = trans('Create ' . ucfirst($this->singularCamelName));
        return view("{$this->view}.create", array_merge(compact('route', 'title'), $externalData));
    }
    ...
    public function store()
    {
        ...
        $data = $this->afterStoreValidate($data);
    ...
    public function show($id)
    {
        ...
        $externalData = $this->external('show');
        $show = $this->model::find($id);
        $title = trans('Show ' . ucfirst($this->singularCamelName));
        return view("$this->view.show", array_merge(compact($this->singularCamelName, 'show', 'title'), $externalData));
    }
    ...
    {
        ${$this->singularCamelName} = $this->model::find($id);
        $externalData = $this->external('edit');
        $edit = $this->model::find($id);
        $route = $this->route;
        $title = trans('Edit ' . ucfirst($this->singularCamelName));
        return view("$this->view.edit", array_merge(compact($this->singularCamelName, 'edit', 'route', 'title'), $externalData));
    }
    ...
    public function update($id)
    {
        ...
        $data = $this->afterUpdateValidate($data);
        $data = $this->uploadFilesIfExist($data);
    ...
    {
        return redirect(route("$this->route.index"))->with('success', 'تم الاضافة بنجاح');
    }

    protected function updated()
    {
        return redirect(route("$this->route.index"))->with('success', trans('تم التعديل بنجاح'));
    }
